...
class CountriesView extends Component {
  ...
  render() {
    ...
    const visitedCountyList = countriesList.filter(
    ...
    return (
      <div className="bg-container">
          ...
          <ul className="ul-container">
            {countriesList.map(eachItem => (
              <CountryItem
                Item={eachItem}
                key={eachItem.id}
                chnageisVisited={this.chnageisVisited}
              />
            ))}
          </ul>
        </div>
        <div className="visited-container">
          ...
          {visitedCountyList.length === 0 ? (
            <p className="para">No Countries Visited Yet!</p>
          ) : (
            <ul className="visited-ul-container">
              {visitedCountyList.map(eachItem => (
                <VisitedCountryItem
                  VisitedItem={eachItem}
                  key={eachItem.id}
